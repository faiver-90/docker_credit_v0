...
document.addEventListener('DOMContentLoaded', () => {
    ...
    fetch(`/credit/offers/?client_id=${clientId}`)
...
async function saveSelectedOffer(offerId, csrfToken, clientId, totalLoanAmount) {
    ...
    try {
        const response = await fetch('/credit/manage_offers/', {
            method: 'POST',
...
function addOfferSelectEventListeners() {
    document.querySelectorAll('.select-offer-button').forEach(button => {
        button.addEventListener('click', async function () {
            ...
            fetch(`/credit/get_card_offer/${offerId}/?car_price=${carPrice}&initial_payment=${initialPayment}&total_loan_amount=${totalLoanAmount}`)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    fetch(`/credit/manage_offers/?client_id=${clientId}`)
        .then(response => response.json())
        .then(data => {
            const offersList = document.getElementById('offers_card_list');
