document.addEventListener('DOMContentLoaded', function () {
    ...
    async function saveField(field) {
        ...
        if (form.id === 'initial_information') {
            url = '/questionnaire/car-form/'; // URL для initial_information
        } else if (form.id === 'additional_information') {
            url = `/questionnaire/load_all_data_client/${clientId}/`; // URL для additional_information
        } else {
...
async function sendInitialForm(csrfToken) {
    try {
        ...
        const response1 = await fetch('/questionnaire/car-form/', { // URL для initial_information
            method: 'POST',
            body: formData1,
            headers: {
                'X-CSRFToken': csrfToken,
...
async function sendAdditionalForm(csrfToken, clientId) {
    try {
        ...
        const response2 = await fetch(`/questionnaire/load_all_data_client/${clientId}/`, { // URL для additional_information
            method: 'POST',
            body: formData2,
            headers: {
                'X-CSRFToken': csrfToken,
