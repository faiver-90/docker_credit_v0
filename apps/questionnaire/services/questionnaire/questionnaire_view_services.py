...
class ClientCreationService:
    ...
    def create_new_client(self, user, dealership_name: Optional[str] = None) -> Tuple[ClientPreData, Dict[str, Any]]:
        forms = {
...
class QuestionnairePostHandler:
    ...
    forms_processing_service = FormProcessingService()

    def handle(self, client_id: int, form_data: Dict[str, Any], ignore_required: bool) -> Dict[str, Any]:
        result = self.forms_processing_service.process_forms(client_id, form_data, ignore_required)
...
class QuestionnaireGetHandler:
    ...
    client_creation_service = ClientCreationService()
    ...
    def handle(self, client_id: Optional[int], id_app_in_system: Optional[str], id_app_bank: Optional[str], user,
        ...
        if client_id:
            context = self.client_data_service.get_client_data(client_id, id_app_in_system, id_app_bank)
            return None, context
        else:
            client, context = self.client_creation_service.create_new_client(user, dealership_name)
